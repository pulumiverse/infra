...
abstract class BaseRepository extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    this._repository = new github.Repository(
      name,
      {
        name: name,
    ...
    pulumi.output(args.teams).apply(teams => {
      teams.forEach(requiredTeam =>
        this.addTeamMembership(name, requiredTeam, args.allTeams),
      );
    });
  }
  ...
  addTeamMembership(
    ...
    if (allTeams.has(requiredTeam)) {
      ...
      let message = `Can't add team ${JSON.stringify(
        requiredTeam,
      )} to ${name}. Did you create the team or is this a typo?`;
  ...
  repositoryArgs.map(repositoryInfo => {
    ...
    switch (repositoryInfo.type) {
      case 'administrative': {
        repositories.set(
          repositoryInfo.name,
          new AdministrativeRepository(repositoryInfo.name, {
      ...
      case 'provider': {
        repositories.set(
          repositoryInfo.name,
          new ProviderRepository(repositoryInfo.name, {
            description: repositoryInfo.description,
            teams: repositoryInfo.teams || [],
            topics: repositoryInfo.topics || [],
            labels: labelArgs || [],
      ...
      case 'information': {
        repositories.set(
          repositoryInfo.name,
          new InformationRepository(repositoryInfo.name, {
            description: repositoryInfo.description,
            teams: repositoryInfo.teams || [],
            topics: repositoryInfo.topics || [],
            labels: labelArgs || [],
...
const kubernetes_sdks = new github.Repository(
  'kubernetes-sdks',
  {
